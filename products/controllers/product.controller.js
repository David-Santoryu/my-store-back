...
exports.modifyProduct = (request, response) => {
  console.log("received modifyProduct request");

  productModel
    .modifyProduct(request.body.info)
    .then((product, error) => {
      if (error) {
        throw error.message;
      }
      ...
      if (product) {
        ...
        console.error("error modifying product");
        return response.status(500);
      }
    })
    .catch((error) => {
